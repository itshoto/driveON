...
class DriveService:
    ...
    def download_file_stream(self, google_file_id, chunk_size=DOWNLOAD_CHUNK_SIZE):
        """Yields the file content in chunks so callers can stream the
        response instead of buffering the whole file (SOP section 21)."""
        request = self._service.files().get_media(fileId=google_file_id)
        buffer = BytesIO()
    ...
    @_persists_refreshed_token
    def delete_file(self, google_file_id):
        self._service.files().delete(fileId=google_file_id).execute()

    @_persists_refreshed_token
    def rename_file(self, google_file_id, new_name):
        self._service.files().update(
            fileId=google_file_id, body={"name": new_name}
        ).execute()

    @_persists_refreshed_token
    def get_file_metadata(self, google_file_id):
        return (
            self._service.files()
            .get(fileId=google_file_id, fields="id,name,mimeType,size,md5Checksum")
            .execute()
        )
